...
for i in range(num_shards):
    ...
    shard = {k: tensors[k] for k in shard_keys}
...
# Write index so transformers can load the sharded model
index = {"metadata": {"total_size": total_size}, "weight_map": weight_map}
index_path = os.path.join(model_dir, "model.safetensors.index.json")
with open(index_path, "w") as f:
    json.dump(index, f, indent=2, sort_keys=True)
print(f"  Index: {len(weight_map)} entries, total_size={total_size}")
...
# Weight index (tells transformers which tensors are in which shard)
COPY model.safetensors.index.json /
EOF

        # Add each shard as a separate layer for parallel pull
